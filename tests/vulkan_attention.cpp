...
int main(int argc,char** argv) {
    ...
    try {
        ...
        for (auto shape : {std::array<int,3>{64,64,64},{64,706,706},{706,64,706},{1024,64,129}}) {
            ...
            ggml_set_name(matching,k==64 ? "laya.amd-low-qk" : "laya.amd-low-pv");
            ...
            for (auto output : {ordinary,matching}) {
                ...
                for (int j=0;j<n;++j) for (int i=0;i<m;i+=31) {
                    ...
                    if (!std::isfinite(actual[j*m+i]) || std::abs(actual[j*m+i]-expected)>0.0002)
                        throw std::runtime_error("Attention matrix mismatch");
                }
        ...
        std::cout << "Vulkan ordinary and matching attention matrices passed\n";
